...
def test_seed_copies_env_and_extra_when_missing(tmp_path):
    src = tmp_path / "src"; (src / "storage").mkdir(parents=True)
    (src / ".env").write_text("DB_PASSWORD=secret\n")
    (src / "storage" / "key.pem").write_text("KEY")
    dst = tmp_path / "wt"; dst.mkdir()
...
def test_prewarm_emits_create_and_populate(tmp_path, monkeypatch):
    ...
    ps._prewarm_volume(r, "webapp-x-api-vendor", "vendor", "webapp-x", "/app/vendor", "example/api-php:dev")
    ...
    assert any("docker run --rm -v webapp-x-api-vendor:/app/vendor example/api-php:dev true" in j for j in joined)
