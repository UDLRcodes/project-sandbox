...
def test_volume_remap_and_toplevel_external():
    compose = {"services": {"api-db": {"image": "mssql",
        "volumes": ["sqlserverdata:/var/opt/mssql/data", "./backups:/backups"]}},
        "volumes": {"sqlserverdata": {}}}
    ov = ps.build_override(compose=compose, project={}, ports={}, instance="demo",
        network_name=None, port_key_by_service={},
    ...
    assert ov["volumes"]["webapp-demo-sqlserverdata"] == {
        "name": "webapp-demo-sqlserverdata", "external": True}


def test_build_args_and_per_instance_image():
    compose = {"services": {"login-app": {"image": "example/login:dev",
        "build": {"context": "."}}}}
    ov = ps.build_override(compose=compose,
        project={"rebuild_per_instance": True, "build_args": {"BACKEND_URL": "http://localhost:${API_WEB}"}},
        ports={"API_WEB": 18080}, instance="demo", network_name=None,
        port_key_by_service={})
...
def test_build_override_namespaces_volumes():
    compose = {"services": {"api-php": {"image": "php",
        "volumes": ["vendor:/app/vendor", "./:/app"]}},
        "volumes": {"vendor": {}}}
    ov = ps.build_override(compose=compose, project={}, ports={}, instance="cir1",
        network_name=None, port_key_by_service={},
